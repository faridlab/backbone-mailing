//! Domain policy for MailingAbTest aggregate
//!
...
/// Domain policy for MailingAbTest — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailingAbTest>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailingAbTestDomainPolicy = PermitAllPolicy<MailingAbTest>;
