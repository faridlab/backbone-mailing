//! Domain policy for MailingFilter aggregate
//!
...
/// Domain policy for MailingFilter — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailingFilter>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailingFilterDomainPolicy = PermitAllPolicy<MailingFilter>;
