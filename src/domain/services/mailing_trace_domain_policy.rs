//! Domain policy for MailingTrace aggregate
//!
...
/// Domain policy for MailingTrace — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<MailingTrace>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type MailingTraceDomainPolicy = PermitAllPolicy<MailingTrace>;
