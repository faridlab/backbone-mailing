//! Domain policy for OptOutReason aggregate
//!
...
/// Domain policy for OptOutReason — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<OptOutReason>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type OptOutReasonDomainPolicy = PermitAllPolicy<OptOutReason>;
